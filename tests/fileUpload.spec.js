...
test("File Upload Test using Playwright inline utility", async ({ page }) => {
    ...
    await fileChooser.setFiles(path.join(__dirname,'../uploads/tmp9cwnga1s.txt'));
...
test("File Upload Test without using Playwright inline utility", async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Choose File' }).setInputFiles(path.join(__dirname,'../uploads/tmp9cwnga1s.txt'));
